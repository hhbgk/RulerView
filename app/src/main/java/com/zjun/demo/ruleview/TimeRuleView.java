...
public class TimeRuleView extends View {
    ...
    private static int[] mPerTextCounts = {
            ...
            10 * 60, 20 * 60,    30 * 60, // 5min/unit: 10min, 20min, 30min
    ...
    private float[] mPerCountScaleThresholds = {
            ...
            0.25f, 0.25f,  0.125f, // 5min/unit: 10min, 20min, 30min
     ...
     * 默认索引值
     */
    private int mPerTextCountIndex = 5;//4;
    /**
     * 一格代表的秒数。默认1min
     */
    ...
    private Paint mPaint, rulerPaint;
...
//    private ScaleGestureDetector mScaleGestureDetector;
    ...
    public interface OnTimeChangedListener{
        void onPrepared(float timeline);
...
//        mHalfWidth = mDeviceWidth >> 1;
        Log.i(tag, "mHalfWidth="+mHalfWidth + ",Device Width="+mDeviceWidth);

        init(context);
//        initScaleGestureDetector(context);

        mTextHalfWidth = mTextPaint.measureText("0") * .5f;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, com.zjun.widget.R.styleable.TimeRuleView);
        bgColor = ta.getColor(com.zjun.widget.R.styleable.TimeRuleView_zjun_bgColor, Color.parseColor("#EEEEEE"));
        gradationColor = ta.getColor(com.zjun.widget.R.styleable.TimeRuleView_zjun_gradationColor, Color.GRAY);
        partHeight = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_partHeight, dp2px(40));
        rulerMarginTop = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_rulerMarginTop, dp2px(50));
        partColor = ta.getColor(com.zjun.widget.R.styleable.TimeRuleView_trv_partColor, Color.parseColor("#F58D24"));
        gradationWidth = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_gradationWidth, dp2px(2));
        rulerWidth = ta.getDimensionPixelSize(com.zjun.widget.R.styleable.TimeRuleView_trv_rulerWidth, dp2px(8));
        secondLen = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_secondLen, dp2px(3));
        minuteLen = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_minuteLen, dp2px(5));
        hourLen = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_hourLen, dp2px(10));
        gradationTextColor = ta.getColor(com.zjun.widget.R.styleable.TimeRuleView_trv_gradationTextColor, Color.GRAY);
        gradationTextSize = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_gradationTextSize, sp2px(12));
        gradationTextGap = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_gradationTextGap, dp2px(2));
        currentTime = ta.getInt(com.zjun.widget.R.styleable.TimeRuleView_trv_currentTime, 0);
        indicatorTriangleSideLen = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_trv_indicatorTriangleSideLen, dp2px(15));
        indicatorWidth = ta.getDimension(com.zjun.widget.R.styleable.TimeRuleView_zjun_indicatorLineWidth, dp2px(1));
        indicatorColor = ta.getColor(com.zjun.widget.R.styleable.TimeRuleView_zjun_indicatorLineColor, Color.RED);
        ta.recycle();
    }
    ...
    private void init(Context context) {
        ...
        rulerPaint.setColor(Color.parseColor("#FFF0F1F8"));
        rulerPaint.setStrokeWidth(rulerWidth);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHalfWidth = (mWidth ) - dp2px(40);
        Log.e(tag, "onMeasure mWidth="+mWidth+", mHalfWidth=" + (mHalfWidth));
        if (mListener != null) {
            mListener.onPrepared(0f);
        }
...
//        mScaleGestureDetector.onTouchEvent(event);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // draw ruler
        canvas.drawLine(0, rulerMarginTop, mWidth, rulerMarginTop, rulerPaint);
        // 刻度
        drawRule(canvas);
...
//        drawTimeParts(canvas);
    ...
    private void drawRule(Canvas canvas) {
        // 移动画布坐标系
        canvas.save();
        canvas.translate(0, rulerMarginTop);
        ...
        final int perTextCount = mPerTextCounts[mPerTextCountIndex];// 10 * 60==>1min/unit: 5min, 10min
...
//                canvas.drawLine(offset, 0, offset, hourLen, mPaint);
                ...
                float minLen = minuteLen;
                // 分刻度
                if (start % 600 == 0) {
                    minLen = minuteLen * 2;
                }
                canvas.drawLine(offset, rulerWidth/2, offset, rulerWidth/2 + minLen, mPaint);
            } else{
                // 秒刻度
//                canvas.drawLine(offset, 0, offset, secondLen, mPaint);
            ...
            if (start % perTextCount == 0) {
                ...
                canvas.save();
                canvas.translate(0, -rulerMarginTop);//rulerWidth
                canvas.rotate(90, offset - mTextHalfWidth, gradationTextGap);
                canvas.drawText(text, offset - mTextHalfWidth, gradationTextGap, mTextPaint);
                canvas.restore();
            }
//            Log.i(tag, "start="+start+",");
    ...
    public int getMiddleLinePosition() {
        Log.i(tag, "mDeviceWidth="+mDeviceWidth+", mWidth=" + mWidth + ", mHalfWidth=" + mHalfWidth);
        return mHalfWidth;// - dp2px(40);
    }
}
